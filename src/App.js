...
const AppContent = () => {
  ...
  const cerrarPeriodo = async (cuenta, montoPago, cierreProx, vencProx) => {
    ...
    const periodo = { inicio: cuenta.cierreAnterior || '2000-01-01', fin: cuenta.cierreActual || '2099-12-31' };
    const consumosPeriodo = movimientos.filter(m => m.cuentaId === cuenta.id && !m.periodoCerrado && m.fecha >= periodo.inicio && m.fecha <= periodo.fin).reduce((s, m) => s + (m.monto || 0), 0);
    ...
    for (const m of movimientos.filter(m => m.cuentaId === cuenta.id && m.fecha >= periodo.inicio && m.fecha <= periodo.fin)) {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme.bg}`}>
        <div className="text-center">
          ...
          <button onClick={signIn} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium">
            Iniciar con Google
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`min-h-screen ${theme.bg}`}>
      ...
      <div className={`sticky top-0 z-40 ${theme.card} border-b ${theme.border}`}>
        <div className="w-full max-w-screen-lg mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <MonityLogo size={32} />
            <span className={`font-bold text-lg ${theme.text}`}>Monity</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {/* Content - Responsive: móvil centrado, web full width */}
      <div className="w-full max-w-screen-lg mx-auto px-4 py-4 pb-24">
        ...
        {tab === 'config' && (
          <div className="space-y-4">
            <h2 className={`text-lg font-bold ${theme.text}`}>Configuración</h2>
            ...
            <div className={`p-4 rounded-xl ${theme.card} border ${theme.border}`}>
              <p className={`text-sm ${theme.textMuted}`}>Usuario: {user?.email}</p>
            </div>
          </div>
        )}
      </div>
